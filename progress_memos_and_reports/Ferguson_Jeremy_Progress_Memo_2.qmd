...
`release_extractor()` finds the first release date and makes this charcater type date.
...
Looping over these functions for each game gives us the current `game_info` data frame, which has 8692 observations, (tentatively) has 4524 unique games, and 204 unique publishers. 
...
At first, I wanted to scrape data from Metacritic, which has both critic scores and user scores. However, I had a very difficult time trying to extract this data, which I believe to be because Metacritic is a dynamic site. Instead, I scraped critic scores from OpenCritic. Though I lost the ability to scrape user data, using OpenCritic made the web scrapping process much more manageable.
