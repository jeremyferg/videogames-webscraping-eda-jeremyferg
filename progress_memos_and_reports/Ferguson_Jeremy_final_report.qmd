---
title: "Video Game Web Scraping and Exploratory Analysis"
subtitle: |
  | Final Project 
...
Irregular data entails that, while the data is not missing from the infobox, the category is formatted in a way that makes web scraping directly from the table difficult. For example, most infoboxes used text to record a game's publisher. However, many games used hyperlinks to record a publisher on the infobox, such as [Elden Ring](https://en.wikipedia.org/wiki/Elden_Ring). Using a regular `rvest` function such as `html_table()` did not accurately parse out the required information for web scraping. More information about extracting concerns can be found on the `videogame_info_wenscraping` R script. This script and all other web scraping scripts have extensive comments that guide the reader through each aspect of the process.
...
Due to the large amount of data sets I collected for this project, I decided to structure these data sets through a relational database design. My goal while structuring this design was to create a system that is simple to navigate, avoids redundant variable observations, and could be populated with new observations/data sets for a future research project. A visual representation of this design is located under the `games_database` folder within the `data` folder. This method of structuring provides a efficient format for connecting data sets with others. For a technical analysis of the approaches used to populate the database, please refer to the `game_relational_database_editing` R script also found in the `games_database` folder. This section will expand on this design alongside providing an overview of each data set within the design.
...
#| fig-cap: Scatterplot of earnings against adjusted closing stock values, grouped by top publishers
...
#| fig-cap: Scatterplot of tournaments against adjusted closing stock values, grouped by top publishers
...
First, we can explore the distribution of game ratings by reviewers through @fig-rating-distr. This histogram is binned by every 5 points. The distribution of ratings follows a fairly normal distribution, with a peak of around 80. It is clear that certain bins, specifically bins including numbers of base 10 (60, 70, 80, etc.), have higher frequencies than other bins. This pattern can be attributed to how the data was collected. Many observations originally were rated out of 5 or out of 10. Scores rated 4 out of 5 in the original review were changed to 80 out of 100. This mutation partially explains why there is a high frequency of ratings within this 80 bin. @tbl-rating-sample-summary shows the statistical summary of this histogram’s distribution. The mean and median are generally similar, with little difference between the standard deviation and IQR as well.
...
```{r}
#| label: tbl-rating-sample-summary
#| tbl-cap: Summary of the rating distribution
...
One may predict that because top publishers generally create higher-quality games compared to other publishers, the ratings of these games will also be higher. @fig-rating-distr-top-publisher, which gives the distribution of reviewer’s ratings by the top publishers, shows a near-identical distribution compared to @fig-rating-distr. Furthermore, @tbl-rating-sample-summary-for-top-publishers also shows an identical median distribution and a mean distribution that is two points higher than @tbl-rating-sample-summary. These observations create the initial implication that video game reviewers score games based on the game’s quality, disregarding  the publisher’s past success.
...
```{r}
#| label: tbl-rating-sample-summary-for-top-publishers
#| tbl-cap : Summary of the rating distribution for games published by top publishers
...
Researchers may be curious about a game rating’s relationship to a publisher’s stocks. Specifically, a researcher may be interested in understanding how increasing the average game rating of a publisher’s game affects the value of their adjusted closing stocks. @fig-ratings-v-adjusted starts this analysis by plotting the adjusted closing stocks of each of the top publishers against the ratings of their games. On an aggregate level, the scatter plot finds a negative correlation between these two variables: increasing the rating of a game decreases a publisher’s stock. From a logical perspective, this correlation is places concern on the legitimacy of this correlation. We can further examine this correlation by subgrouping ratings by publishers. @fig-ratings-v-adjusted-groupby-top-publishers shows that most publishers do not show a correlation between adjusted closing stocks and game ratings. If there is a correlation, the magnitude of this relationship is negligible. Therefore, a researcher should be hesitant to correlate ratings with these adjusted closing stocks.

```{r}
#| label: fig-ratings-v-adjusted
...
This project provides a valuable resource to those in the video game industry as an excellent data source for database analysis and a useful assortment of research ideas related to the wrangled data. Not only do researchers now have access to an extensive collection of publishers’ business actions and their relation to stocks, but this database also has the potential to be easily updated with additional observations or expanded with additional data sets. Although the data exploration of this project was simplistic, I believe I was able to introduce insightful questions or curiosity related to the data that ignited further exploration. These questions give me confidence that the database can effectively combine data sets to create meaningful observations.

For future research, I suggest expanding on the research topics and questions discussed during the exploration process. Depending on the needs of a company or the curiosity of a researcher, certain topics hold strong value to a company’s outcomes. For example, if a publisher seeks to understand the quantitative importance of strong game reviews, further exploring the video game reviews data set and its relation to publishers’ stocks is essential. I also encourage researchers to modify the database to their own uniqueness. For example, a researcher could expand, remove, or modify the publisher stocks collected in the database by modifying the chosen stock names in the `game_relational_database_editing` R script. These actions would further allow researchers or those in the videogame industry to tailor their investigation to their company’s current needs. 
